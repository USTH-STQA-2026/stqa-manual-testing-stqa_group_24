...
| TC-05 | Login | Displays error message: `Vui lòng nhập email và mật khẩu`. | Displayed error message under the email input field: `Vui lòng nhập email và mật khẩu`. | Pass | ![Blank Email](./assets/REQ-01/Blank-email.png) | - |
| TC-06 | Login | Displays error message: `Vui lòng nhập email và mật khẩu`. | Displayed error message under the email input field: `Vui lòng nhập email và mật khẩu`. | Pass | ![Blank Password](./assets/REQ-01/Blank-pass.png) | - |
| TC-07 | Login | Displays error message: `Không tìm thấy thành viên`. | Displayed error message under the email input field: `Không tìm thấy thành viên`. | Pass | ![Wrong Email and Password](./assets/REQ-01/Wrong-Email-Pass.png) | - |

### REQ-04: Borrow Book

| Mã TC | Nhóm chức năng | Kết quả mong đợi (tóm tắt) | Kết quả thực tế | Kết luận | Minh chứng | Bug |
|-------|---------------|---------------------------|-----------------|---------|-----------|----|
| TC-08 | Borrow Book | Successful book borrowing with correct status transition and 14-day loan period calculation. | - The member borrowed BOOK001 successfully.<br>- Book status transitioned to `Borrowed`.<br>- A 14-day loan period was added from the borrow date.<br>- The system displayed `Mượn sách thành công!`. | Pass | ![](./assets/REQ-04/08-1.png)<br>![](./assets/REQ-04/08-2.png)<br>![](./assets/REQ-04/08-3.png)<br>![](./assets/REQ-04/08-4.png) | - |
...
| TC-11 | Borrow Book | The system denies the borrowing request if the member status is suspended. | The system displayed a red error banner: `Thành viên đã hết hạn. Không thể mượn sách.` instead of a suspended-member message. | Fail | ![](./assets/REQ-04/11-1.png)<br>![](./assets/REQ-04/11-2.png)<br>![](./assets/REQ-04/11-3.png) | BUG-01 |
| TC-12 | Borrow Book | The system denies the borrowing request if the member status is expired. | The system displayed a red error banner: `Thành viên đã hết hạn. Không thể mượn sách.` | Pass | ![](./assets/REQ-04/12-1.png)<br>![](./assets/REQ-04/12-2.png)<br>![](./assets/REQ-04/12-3.png) | - |
| TC-13 | Borrow Book | The system denies the borrowing request because the member has already reached the maximum limit of 3 borrowed books. | The system allowed the borrowing transaction and displayed `Mượn sách thành công!`. | Fail | ![](./assets/REQ-04/13-1.png)<br>![](./assets/REQ-04/13-2.png)<br>![](./assets/REQ-04/13-3.png)<br>![](./assets/REQ-04/13-4.png)<br>![](./assets/REQ-04/13-5.png) | BUG-02 |
...
| TC-15 | View Book List | Books with different initial statuses are displayed correctly. | BOOK001 was displayed as `Có sẵn`, BOOK003 was displayed as `Đã mượn`, and BOOK007 was displayed as `Thất lạc`. | Pass | ![TC-15 status available borrowed](./assets/REQ-02/TC-15-status-available-borrowed.png)<br>![TC-15 status lost](./assets/REQ-02/TC-15-status-lost.png) | - |
...
| Mã TC | Nhóm chức năng | Kết quả mong đợi (tóm tắt) | Kết quả thực tế | Kết luận | Minh chứng | Bug |
|-------|---------------|---------------------------|-----------------|---------|-----------|----|
| TC-24 | Return Book | A member can return their own currently borrowed book. | After logging in as MEM006 and returning BR003 for BOOK013, the system displayed `Trả sách thành công.` BOOK013 then changed back to `Có sẵn` in the Books tab. | Pass | ![TC-24 return BR003 success](./assets/REQ-05/TC-24-return-br003-success.png)<br>![TC-24 BOOK013 available after return](./assets/REQ-05/TC-24-book013-available-after-return.png) | - |
| TC-25 | Return Book | Returning an overdue book displays an overdue warning. | BR001 was overdue with due date 15/09/2024. After returning BR001, the system only displayed `Trả sách thành công.` and did not display any overdue warning. BOOK003 changed back to `Có sẵn`. | Fail | ![TC-25 before returning overdue BR001](./assets/REQ-05/TC-25-before-return-overdue-br001.png)<br>![TC-25 after returning overdue BR001 without warning](./assets/REQ-05/TC-25-after-return-overdue-br001-no-warning.png)<br>![TC-25 BOOK003 available after return](./assets/REQ-05/TC-25-book003-available-after-return.png) | BUG-04 |
| TC-26 | Return Book / Borrow Record Lookup | A member must not be able to view or return another member’s borrowed book. | MEM002 searched for MEM006 and the system displayed BR003 of MEM006 with a **Return Book** button. After clicking it, the system displayed `Trả sách thành công.` and BR003 changed to `Đã trả`. | Fail | ![TC-26 member views other record BR003](./assets/REQ-05/TC-26-member-view-other-record-br003.png)<br>![TC-26 member returns other record successfully](./assets/REQ-05/TC-26-member-return-other-record-success.png) | BUG-05 |
| TC-27 | Return Book | An already returned record cannot be returned again. | BR004 was displayed with status `Đã trả`, and no **Return Book** button was available for BR004. | Pass | ![TC-27 returned record BR004 no return button](./assets/REQ-05/TC-27-returned-record-br004-no-return-button.png) | - |

### REQ-06: Overdue Handling

| Mã TC | Nhóm chức năng | Kết quả mong đợi (tóm tắt) | Kết quả thực tế | Kết luận | Minh chứng | Bug |
|-------|---------------|---------------------------|-----------------|---------|-----------|----|
| TC-28 | Overdue Handling | The librarian can trigger overdue checking. | After the librarian clicked **Kiểm tra sách quá hạn**, the system displayed `Đã cập nhật: 2 phiếu mượn quá hạn.` and marked overdue records. | Pass | ![TC-28 overdue check success](./assets/REQ-06/TC-28-check-overdue-success.png) | - |
| TC-29 | Overdue Handling | Overdue borrow records are marked as `Quá hạn`. | Before overdue checking, BR001 and BR003 were still active borrowed records. After clicking **Kiểm tra sách quá hạn**, BR001 and BR003 were marked as `Quá hạn`. | Pass | ![TC-29 before overdue check](./assets/REQ-06/TC-29-before-overdue-check.png)<br>![TC-29 after overdue check](./assets/REQ-06/TC-29-after-overdue-check.png) | - |
...
| TC-31 | Overdue Handling | A member can view their own overdue record after overdue checking. | After the librarian performed overdue checking, MEM002 could view their own BR001 marked as `Quá hạn`. | Pass | ![TC-31 librarian check overdue](./assets/REQ-06/TC-31-librarian-check-overdue.png)<br>![TC-31 member own overdue record](./assets/REQ-06/TC-31-member-own-overdue-record.png) | - |

### REQ-07: Member Management

| Mã TC | Nhóm chức năng | Kết quả mong đợi (tóm tắt) | Kết quả thực tế | Kết luận | Minh chứng | Bug |
|-------|---------------|---------------------------|-----------------|---------|-----------|----|
| TC-32 | Member Management | The librarian can add a new member with valid email format `user@domain.com`. | After entering full name, phone number, and valid email `user@domain.com`, the system displayed `Email không hợp lệ.` and did not create the new member. | Fail | ![TC-32 valid email rejected](./assets/REQ-07/TC-32-valid-email-rejected.png) | BUG-06 |
...
| TC-34 | Member Management | The system rejects email `user@domain` because it has no dot in the domain part. | After entering invalid email `user@domain`, the system still created a new member and displayed `Thêm thành viên thành công! Mã: MEM007`. | Fail | ![TC-34 invalid email input](./assets/REQ-07/TC-34-invalid-email-user-domain-input.png)<br>![TC-34 invalid email accepted](./assets/REQ-07/TC-34-invalid-email-user-domain-accepted.png) | BUG-07 |
| TC-35 | Member Management | The system rejects an existing email with a duplicate email error. | After entering existing email `ba.nguyen@email.com`, the system displayed `Email không hợp lệ.` instead of an email duplication error. | Fail | ![TC-35 existing email input](./assets/REQ-07/TC-35-existing-email-input.png)<br>![TC-35 existing email wrong error](./assets/REQ-07/TC-35-existing-email-wrong-error.png) | BUG-08 |
