...
| TC-34 | Member Management | The system rejects adding a new member when required fields are missing. | After leaving the full name field blank and entering the remaining information, the system blocked the new member creation and displayed `Họ tên không được để trống.` | Pass | ![TC-34 missing required field blocked](./assets/REQ-07/TC-34-missing-required-field-blocked.png) | - |
| TC-35 | Member Management | The system rejects an existing email with a duplicate email error. | After entering existing email `ba.nguyen@email.com`, the system displayed `Email không hợp lệ.` instead of an email duplication error. | Fail | ![TC-35 existing email input](./assets/REQ-07/TC-35-existing-email-input.png)<br>![TC-35 existing email wrong error](./assets/REQ-07/TC-35-existing-email-wrong-error.png) | BUG-07 |
